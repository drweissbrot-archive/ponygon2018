...
var linkArea = document.querySelector('#link-area');
var baseLink = linkArea.getAttribute('data-base');
var linkEl = linkArea.querySelector('a');
var newGameArea = document.querySelector('.new-game');
...
var TicTacToe = function () {
	...
	_createClass(TicTacToe, [{
		...
		value: function startGame() {
			...
			axios.get('/play/tic-tac-toe/start').then(function (res) {
				_this.auth = res.data.auth;
				_this.id = res.data.game_id;
				_this.player = 'X';

				_this.applyLink(res.data.game_id);
				_this.applyTurn('waiting');

				_this.subscribe();
			}).catch(function (err) {
				_this.applyTurn('error');
				console.error(err.response);
			});
		}
	}, {
		key: 'startGameFromHash',
		value: function startGameFromHash() {
			this.player = 'X';
		...
		key: 'registerAsPlayerTwo',
		value: function registerAsPlayerTwo() {
			var _this2 = this;

			console.info('Registering as second player', this.player);

			axios.get('/play/tic-tac-toe/register/' + this.id + '/' + this.player).then(function (res) {
				_this2.auth = res.data.auth;

				_this2.applyTurn('X');
			}).catch(function (err) {
				_this2.applyTurn('error');
				console.error(err.response);
			});
		}
	}, {
		...
		value: function subscribe() {
			...
			Echo.channel(this.channel).listen('Game\\TicTacToe\\MoveMade', function (e) {
				_this3.applyTurn(e.state.turn);
				_this3.applyState(e.state);
			}).listen('Game\\TicTacToe\\UserRegistered', function (e) {
				_this3.hideLinkArea();

				_this3.applyTurn(e.state);
			}).listen('Game\\TicTacToe\\NewGameStarted', function (e) {
				console.info('New game started...', e.byPlayer);
				_this3.id = e.newId;

				_this3.subscribe();
				_this3.hideNewGameButton();

				_this3.applyState('empty');

				_this3.swapPlayer();

				if (_this3.player === e.byPlayer) {
					_this3.registerAsPlayerTwo();
				}
			});
		}
	}, {
		...
		value: function parseNodes() {
			...
			try {
				for (var _iterator = document.querySelectorAll('[data-node]')[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
					node = _step.value;

					var coordinate = node.getAttribute('data-node');

					this.nodes[coordinate] = node;

					node.addEventListener('click', function (e) {
						_this4.makeMove(e.target.getAttribute('data-node'));
					});
				}
			} catch (err) {
		...
		value: function applyState(state) {
			for (var coordinate in this.nodes) {
				this.nodes[coordinate].innerText = state == 'empty' ? '' : state[coordinate];
			}
		}
	}, {
		key: 'applyTurn',
		value: function applyTurn(move) {
			switch (move) {
				case this.player:
		...
		key: 'showLinkArea',
		value: function showLinkArea() {
			linkArea.classList.add('--active');
		}
	}, {
		key: 'hideLinkArea',
		value: function hideLinkArea() {
		...
		value: function startNewGame() {
			...
			axios.post('/play/tic-tac-toe/start-new/' + this.id, {
				...
				_this6.id = res.data.game_id;
				_this6.auth = res.data.auth;

				_this6.applyTurn('waiting');
				_this6.applyState('empty');
			}).catch(function (err) {
				console.error(err.response);
				_this6.applyTurn('error');
			});
		}
	}, {
		key: 'addStartNewButtonListener',
		value: function addStartNewButtonListener() {
			var _this7 = this;

			newGameArea.querySelector('[data-action="new-game"]').addEventListener('click', function (e) {
				e.preventDefault();
				_this7.startNewGame();
			});
		}
